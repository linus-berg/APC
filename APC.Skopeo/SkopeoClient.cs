...
public class SkopeoClient {
  public async Task CopyToOci(string image, string oci_dir) {
    ...
    Command cmd = Cli.Wrap("skopeo").WithWorkingDirectory(oci_dir).WithArguments(args => {
      args.Add("copy");
      args.Add("--quiet");
      args.Add("--dest-shared-blob-dir");
      args.Add("shared");
      args.Add(image);
      args.Add($"oci:repo:{uri.GetComponents(UriComponents.Host | UriComponents.Path, UriFormat.Unescaped)}");
    });
    StringBuilder sb = new();
    Console.WriteLine($"Pulling {image}");
    try {
      CommandResult result = await (cmd | PipeTarget.ToStringBuilder(sb)).ExecuteAsync();
  ...
  private async Task<T> MapCommandOutput<T>(Command cmd) {
    ...
    if (result.ExitCode != 0) throw new ApplicationException("Skopeo failed");
